...
public class FileController {
    ...
    @RequestMapping(value = "/{domain}/json/{id}", method = RequestMethod.GET, produces = MediaType.TEXT_PLAIN_VALUE)
    @ResponseBody
    public FileSystemResource getJSON(@PathVariable String domain, @PathVariable String id) {
    ...
    @RequestMapping(value = "/{domain}/json/{id}", method = RequestMethod.DELETE)
    @ResponseBody
    public void deleteJSON(@PathVariable String domain, @PathVariable String id) {
        DomainConfig domainConfig = domainConfigCache.getConfigForDomainName(domain);
        if (domainConfig == null || !domainConfig.getChannels().contains(ValidatorChannel.FORM)) {
            throw new NotFoundException();
        }
    ...
    public void cleanUpFiles() {
        ...
        if (reportFolder != null) {
            ...
            if (files != null) {
                for (File file: files) {
                    if (!handleReportFile(file, "ITB-", currentMillis, config.getMinimumCachedInputFileAge())) {
                        handleReportFile(file, "TAR-", currentMillis, config.getMinimumCachedReportFileAge());
                    }
                }
            }
        }
