...
function getReportData(jsonID) {
	getReport(jsonID);
	getResultReport(jsonID);
}
function getReport(jsonID) {
	$.get("json/"+jsonID, function(data) {
		itbReportData = data;
		$.ajax({
			url: "json/"+jsonID,
			type: 'DELETE'
...
function getResultReport(jsonID) {
	$.ajax({
		url: "report/"+jsonID+"/xml",
		type: 'GET',
    ...
    ajax.open("GET", "report/"+jsonID+"/pdf", true);
...
function setCode(reportItemElement) {
	reportLoad.done(function() {
		var editorContent = $('#json-content-pane');
		editorContent.empty();
		...
		$('#json-content-modal').modal('show');
		$('#json-content-modal').on('shown.bs.modal', function() {
			cm.refresh();
